...
ysD3D10Device::~ysD3D10Device() { /* void */ }
...
ysError ysD3D10Device::InitializeDevice() {
    ...
    HRESULT result =
            D3D10CreateDevice(nullptr, D3D10_DRIVER_TYPE_HARDWARE, nullptr,
                              flags, D3D10_SDK_VERSION, &m_device);
...
ysError ysD3D10Device::CreateRenderingContext(ysRenderingContext **context,
                                              ysWindow *window) {
    ...
    *context = nullptr;
    ...
    ysD3D10Context *newContext =
            m_renderingContexts.NewGeneric<ysD3D10Context>();
    ...
    HRESULT result;
    result = factory->CreateSwapChain(pDXGIDevice, &swapChainDesc,
    ...
    if (!m_rasterizerState) {
        ...
        m_device->OMSetBlendState(g_pBlendStateNoBlend, blendFactor,
                                  sampleMask);
...
ysError ysD3D10Device::UpdateRenderingContext(ysRenderingContext *context) {
    ...
    ysD3D10RenderTarget *attachedTarget = static_cast<ysD3D10RenderTarget *>(
...
ysError ysD3D10Device::DestroyRenderingContext(ysRenderingContext *&context) {
    ...
    if (context) {
        YDS_NESTED_ERROR_CALL(SetContextMode(
                context, ysRenderingContext::ContextMode::Windowed));
...
ysError ysD3D10Device::SetContextMode(ysRenderingContext *context,
...
ysError ysD3D10Device::CreateOnScreenRenderTarget(ysRenderTarget **newTarget,
    ...
    *newTarget = nullptr;
    ...
    ysD3D10RenderTarget *newRenderTarget =
            m_renderTargets.NewGeneric<ysD3D10RenderTarget>();

    ysError result = CreateD3D10OnScreenRenderTarget(newRenderTarget, context,
                                                     depthBuffer);
...
ysError ysD3D10Device::CreateOffScreenRenderTarget(
    ...
    *newTarget = nullptr;
    ...
    ysD3D10RenderTarget *d3d10Target =
            m_renderTargets.NewGeneric<ysD3D10RenderTarget>();

    ysError result = CreateD3D10OffScreenRenderTarget(
            d3d10Target, width, height, format, colorData, depthBuffer);
...
ysError ysD3D10Device::CreateSubRenderTarget(ysRenderTarget **newTarget,
...
ysError ysD3D10Device::ResizeRenderTarget(ysRenderTarget *target, int width,
    ...
    for (int i = 0; i < MaxRenderTargets; ++i) {
        if (target == m_activeRenderTarget[i]) { SetRenderTarget(target, i); }
    }
...
ysError ysD3D10Device::SetRenderTarget(ysRenderTarget *newTarget, int slot) {
    ...
    if (newTarget) {
        ysD3D10RenderTarget *d3d10Target =
                static_cast<ysD3D10RenderTarget *>(newTarget);
        ysD3D10RenderTarget *realTarget =
                (newTarget->GetType() == ysRenderTarget::Type::Subdivision)
                        ? static_cast<ysD3D10RenderTarget *>(
                                  newTarget->GetParent())
                        : d3d10Target;

        if (realTarget != m_activeRenderTarget[slot]) {
            GetDevice()->OMSetRenderTargets(1, &realTarget->m_renderTargetView,
                                            realTarget->m_depthStencil);
        }
...
ysError ysD3D10Device::ClearBuffers(const float *clearColor) {
    ...
    for (int i = 0; i < MaxRenderTargets; ++i) {
        ...
        if (m_activeRenderTarget != nullptr) {
            ysD3D10RenderTarget *renderTarget =
                    static_cast<ysD3D10RenderTarget *>(actualRenderTarget);
            m_device->ClearRenderTargetView(renderTarget->m_renderTargetView,
                                            clearColor);
            if (renderTarget->m_hasDepthBuffer) {
                m_device->ClearDepthStencilView(renderTarget->m_depthStencil,
...
// Vertex Buffers
ysError ysD3D10Device::CreateVertexBuffer(ysGPUBuffer **newBuffer, int size,
    ...
    *newBuffer = nullptr;
    ...
    ysD3D10GPUBuffer *newD3D10Buffer =
            m_gpuBuffers.NewGeneric<ysD3D10GPUBuffer>();
...
ysError ysD3D10Device::CreateIndexBuffer(ysGPUBuffer **newBuffer, int size,
                                         char *data, bool mirrorToRam) {
...
ysError ysD3D10Device::CreateConstantBuffer(ysGPUBuffer **newBuffer, int size,
                                            char *data, bool mirrorToRam) {
...
ysError ysD3D10Device::UseVertexBuffer(ysGPUBuffer *buffer, int stride,
                                       int offset) {
    ...
    if (buffer) {
        ...
        if (d3d10Buffer->m_bufferType == ysGPUBuffer::GPU_DATA_BUFFER &&
            (buffer != m_activeVertexBuffer ||
             stride != d3d10Buffer->m_currentStride)) {
            GetDevice()->IASetVertexBuffers(0, 1, &d3d10Buffer->m_buffer,
                                            &ustride, &uoffset);
        }
    } else {
...
ysError ysD3D10Device::UseIndexBuffer(ysGPUBuffer *buffer, int offset) {
    ...
    if (buffer) {
        ...
        if (d3d10Buffer->m_bufferType == ysGPUBuffer::GPU_INDEX_BUFFER &&
            buffer != m_activeIndexBuffer) {
            GetDevice()->IASetIndexBuffer(d3d10Buffer->m_buffer,
                                          DXGI_FORMAT_R16_UINT, uoffset);
        }
    } else {
...
ysError ysD3D10Device::EditBufferDataRange(ysGPUBuffer *buffer, char *data,
                                           int size, int offset) {
    ...
    GetDevice()->UpdateSubresource(d3d10Buffer->m_buffer, 0, &box, data,
                                   buffer->GetSize(), 0);
...
ysError ysD3D10Device::EditBufferData(ysGPUBuffer *buffer, char *data) {
    ...
    GetDevice()->UpdateSubresource(d3d10Buffer->m_buffer, 0, nullptr, data, 0,
                                   0);
...
ysError ysD3D10Device::CreateVertexShader(ysShader **newShader,
    ...
    *newShader = nullptr;
    ...
    result = m_device->CreateVertexShader(shaderByteCode, shaderSize,
                                          &vertexShader);
    if (FAILED(result)) {
        free(shaderByteCode);
...
ysError ysD3D10Device::CreatePixelShader(ysShader **newShader,
                                         const wchar_t *shaderFilename,
                                         const wchar_t *compiledFilename,
    ...
    *newShader = nullptr;
    ...
    result = m_device->CreatePixelShader(shaderByteCode, shaderSize,
                                         &pixelShader);
    if (FAILED(result)) {
        free(shaderByteCode);
        return YDS_ERROR_RETURN(ysError::CouldNotCreateShader);
...
ysError ysD3D10Device::DestroyShaderProgram(ysShaderProgram *&program,
                                            bool destroyShaders) {
...
ysError ysD3D10Device::CreateShaderProgram(ysShaderProgram **newProgram) {
    ...
    *newProgram = nullptr;
    ...
    ysD3D10ShaderProgram *newD3D10Program =
            m_shaderPrograms.NewGeneric<ysD3D10ShaderProgram>();
    *newProgram = static_cast<ysShaderProgram *>(newD3D10Program);

    return YDS_ERROR_RETURN(ysError::None);
}

ysError ysD3D10Device::AttachShader(ysShaderProgram *program,
                                    ysShader *shader) {
...
ysError ysD3D10Device::UseShaderProgram(ysShaderProgram *program) {
    ...
    ysD3D10ShaderProgram *d3d10Program =
            static_cast<ysD3D10ShaderProgram *>(program);
    ysD3D10ShaderProgram *currentProgram =
            static_cast<ysD3D10ShaderProgram *>(m_activeShaderProgram);
    ysD3D10Shader *vertexShader =
            (program) ? d3d10Program->GetShader(ysShader::ShaderType::Vertex)
                      : nullptr;
    ysD3D10Shader *fragmentShader =
            (program) ? d3d10Program->GetShader(ysShader::ShaderType::Pixel)
                      : nullptr;

    if (d3d10Program == currentProgram) return YDS_ERROR_RETURN(ysError::None);

    ysD3D10Shader *currentVertexShader =
            (currentProgram)
                    ? currentProgram->GetShader(ysShader::ShaderType::Vertex)
                    : nullptr;
    ysD3D10Shader *currentPixelShader =
            (currentProgram)
                    ? currentProgram->GetShader(ysShader::ShaderType::Pixel)
                    : nullptr;

    if (vertexShader != currentVertexShader) {
        GetDevice()->VSSetShader((vertexShader) ? vertexShader->m_vertexShader
                                                : nullptr);
    }

    if (fragmentShader != currentPixelShader) {
        GetDevice()->PSSetShader(
...
ysError
ysD3D10Device::CreateInputLayout(ysInputLayout **newInputLayout,
                                 ysShader *shader,
    ...
    *newInputLayout = nullptr;
    ...
    result = m_device->CreateInputLayout(
            descArray, nChannels, d3d10Shader->m_shaderByteCode,
            SIZE_T(d3d10Shader->m_shaderSize), &layout);

    if (FAILED(result)) {
        return YDS_ERROR_RETURN(ysError::IncompatibleInputFormat);
    }

    ysD3D10InputLayout *newD3D10Layout =
            m_inputLayouts.NewGeneric<ysD3D10InputLayout>();
...
// Textures
ysError ysD3D10Device::CreateTexture(ysTexture **newTexture,
                                     const wchar_t *fname) {
...
ysError ysD3D10Device::CreateTexture(ysTexture **texture, int width, int height,
...
ysError ysD3D10Device::UpdateTexture(ysTexture *texture,
                                     const unsigned char *buffer) {
    YDS_ERROR_DECLARE("UpdateTexture");
    return YDS_ERROR_RETURN(ysError::NotImplemented);
}

ysError ysD3D10Device::CreateAlphaTexture(ysTexture **texture, int width,
                                          int height,
                                          const unsigned char *buffer) {
    YDS_ERROR_DECLARE("CreateTexture");
...
ysError ysD3D10Device::UseTexture(ysTexture *texture, int slot) {
    ...
    if (texture != m_activeTextures[slot].Texture) {
        ...
        GetDevice()->PSSetShaderResources(
                slot, 1, (texture) ? &d3d10Texture->m_resourceView : &nullView);
    }
...
ysError ysD3D10Device::UseRenderTargetAsTexture(ysRenderTarget *texture,
                                                int slot) {
    YDS_ERROR_DECLARE("UseTexture");
    ...
    ysD3D10RenderTarget *d3d10Texture =
            static_cast<ysD3D10RenderTarget *>(texture);
...
ysError ysD3D10Device::DestroyTexture(ysTexture *&texture) {
    ...
    if (d3d10Texture->m_renderTargetView)
        d3d10Texture->m_renderTargetView->Release();
...
DXGI_FORMAT ysD3D10Device::ConvertInputLayoutFormat(
...
ysError
ysD3D10Device::CreateD3D10DepthBuffer(ID3D10DepthStencilView **newDepthStencil,
...
ysError
ysD3D10Device::DestroyD3D10DepthBuffer(ID3D10DepthStencilView *&depthStencil) {
...
ysError
ysD3D10Device::CreateD3D10OnScreenRenderTarget(ysRenderTarget *newTarget,
                                               ysRenderingContext *context,
                                               bool depthBuffer) {
    ...
    result = d3d10Context->m_swapChain->GetBuffer(0, __uuidof(ID3D10Texture2D),
                                                  (LPVOID *) (&backBuffer));
    if (FAILED(result)) {
    ...
    if (depthBuffer) {
        ysError depthResult;
        depthResult = CreateD3D10DepthBuffer(
                &newDepthStencil, context->GetWindow()->GetGameWidth(),
                context->GetWindow()->GetGameHeight(), 1, 0);
...
ysError ysD3D10Device::CreateD3D10OffScreenRenderTarget(
    ...
    if (depthBuffer) {
        ysError depthResult;
        depthResult =
                CreateD3D10DepthBuffer(&newDepthStencil, width, height, 1, 0);
...
ysError ysD3D10Device::DestroyD3D10RenderTarget(ysRenderTarget *target) {
    ...
    ysD3D10RenderTarget *d3d10Target =
            static_cast<ysD3D10RenderTarget *>(target);
    if (d3d10Target->m_renderTargetView != nullptr)
        d3d10Target->m_renderTargetView->Release();
    if (d3d10Target->m_depthStencil != nullptr)
        d3d10Target->m_depthStencil->Release();
    if (d3d10Target->m_resourceView != nullptr)
        d3d10Target->m_resourceView->Release();
