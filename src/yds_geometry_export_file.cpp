...
int ysGeometryExportFile::GetVertexSize(ysInterchangeObject *object, const VertexInfo *info) {
	int numUVChannels = info->IncludeUVs ? info->UVChannels : 0;
	bool includeNormals = info->IncludeNormals;
	bool includeTangents = info->IncludeTangents;
		...
		if (numRequiredUVChannels > 0) {
			for (int channel = 0; channel < numRequiredUVChannels; channel++) {
				if (channel >= numUVChannels) {
					...
					float u = object->UVChannels[channel].Coordinates[UVCoordinates[channel][vert]].x;
					float v = object->UVChannels[channel].Coordinates[UVCoordinates[channel][vert]].y;
...
ysError ysGeometryExportFile::WriteObject(ysInterchangeObject *object, const VertexInfo *info) {
	...
	if (!m_file.is_open()) return YDS_ERROR_RETURN(ysError::YDS_NO_FILE);
	if (object == nullptr) return YDS_ERROR_RETURN(ysError::YDS_INVALID_PARAMETER);
