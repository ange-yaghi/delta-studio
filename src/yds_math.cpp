...
float ysMath::UniformRandom(float range) {
    ...
    r = r > MAX_RAND ? MAX_RAND : r;

    return range * r;
}

int ysMath::UniformRandomInt(int range) { return rand() % range; }
...
ysGeneric ysMath::Lerp(const ysGeneric &a, const ysGeneric &b, float s) {
    ...
    return ysMath::Add(ysMath::Mul(a, s_comp), ysMath::Mul(b, s_v));
}
...
float ysMath::GetQuatX(const ysQuaternion &v) { return v.m128_f32[1]; }

float ysMath::GetQuatY(const ysQuaternion &v) { return v.m128_f32[2]; }

float ysMath::GetQuatZ(const ysQuaternion &v) { return v.m128_f32[3]; }

float ysMath::GetQuatW(const ysQuaternion &v) { return v.m128_f32[0]; }
...
ysVector ysMath::Cross2D(const ysVector &v) {
    return _mm_mul_ps(_mm_shuffle_ps(v, v, _MM_SHUFFLE(3, 2, 0, 1)),
                      {-1.0f, 1.0f, 1.0f, 1.0f});
}
...
ysVector ysMath::Abs(const ysVector &a) { return ComponentMax(a, Negate(a)); }
...
ysQuaternion ysMath::QuatMultiply(const ysQuaternion &q1,
                                  const ysQuaternion &q2) {
    ...
    ysGeneric result =
            _mm_add_ps(_mm_add_ps(prod1, prod2), _mm_add_ps(prod3, prod4));
...
ysQuaternion ysMath::QuatTransform(const ysQuaternion &q, const ysVector &v) {
    ...
    ysQuaternion p_trans = QuatMultiply(QuatMultiply(q, p), q_inv);
...
ysQuaternion ysMath::QuatTransformInverse(const ysQuaternion &q,
                                          const ysVector &v) {
    ysGeneric p = _mm_shuffle_ps(v, v, _MM_SHUFFLE(2, 1, 0, 3));// wxyz
    p = _mm_and_ps(p, ysMath::Constants::MaskOffX);
    ...
    ysQuaternion p_trans = QuatMultiply(QuatMultiply(q_inv, p), q);
...
ysQuaternion ysMath::QuatAddScaled(const ysQuaternion &q, const ysVector &vec,
                                   float scale) {
...
ysMatrix ysMath::LoadMatrix(const ysVector &r1, const ysVector &r2,
                            const ysVector &r3, const ysVector &r4) {
    ysMatrix r;
    r.rows[0] = r1;
    r.rows[1] = r2;
    r.rows[2] = r3;
...
ysMatrix ysMath::LoadMatrix(const ysQuaternion &quat) {
    ...
    ysGeneric nq =
            _mm_sub_ps(ysMath::Constants::Zero, q);// nq = [-w, -x, -y, -z]
    ysGeneric qq = _mm_add_ps(q, q);               // qq = [2w, 2x, 2y, 2z]
    ysGeneric q2 = _mm_mul_ps(qq, q);// q2 = [2w^2, 2x^2, 2y^2, 2z^2]

    ysGeneric xxxy =
            _mm_shuffle_ps(q, q, _MM_SHUFFLE(2, 1, 1, 1));// [x, x, x, y]
    ysGeneric zyyz =
            _mm_shuffle_ps(qq, qq, _MM_SHUFFLE(3, 2, 2, 3));// [2z, 2y, 2y, 2z]
    ysGeneric yzzx =
            _mm_shuffle_ps(qq, qq, _MM_SHUFFLE(1, 3, 3, 2));// [2y, 2z, 2z, 2x]
    ysGeneric wwww =
            _mm_shuffle_ps(q, nq, _MM_SHUFFLE(0, 0, 0, 0));// [w, w, -w, -w]

    ysGeneric i1 = _mm_mul_ps(xxxy, zyyz);// [2xz, 2xy, 2xy, 2yz]
    ysGeneric i2 = _mm_mul_ps(yzzx, wwww);// [2yw, 2zw, -2zw, -2xw]
    ysGeneric calc1 =
            _mm_add_ps(i1, i2);// [2xz + 2yw, 2xy + 2zw, 2xy - 2zw, 2yz - 2xw]
    ...
    ysGeneric calc2 = _mm_sub_ps(ysMath::Constants::One,
                                 _mm_add_ps(y2_x2_x2_w2, z2_z2_y2_w2));
    calc2 = _mm_and_ps(calc2, ysMath::Constants::MaskOffW.vector);

    // Stage 3
    ysGeneric calc3 =
            _mm_sub_ps(i1, i2);// [2xz - 2yw, 2xy - 2zy, 2xy + 2zw, 2yz + 2xw]
    ...
    ysGeneric asm1 = _mm_shuffle_ps(
            calc2, calc3,
            _MM_SHUFFLE(0, 2, 3,
                        0));// [1 - 2(y^2 + z^2), 0, 2xy - 2zw, 2xz + 2yw]
    asm1 = _mm_shuffle_ps(
            asm1, asm1,
            _MM_SHUFFLE(1, 3, 2,
                        0));// [1 - 2(y^2 + z^2), 2xy - 2zw, 2xz + 2yw, 0]

    ysGeneric asm2 = _mm_shuffle_ps(
            calc2, calc3,
            _MM_SHUFFLE(3, 1, 3,
                        1));// [1 - 2(x^2 + z^2), 0, 2xy + 2zw, 2yz - 2xw]
    asm2 = _mm_shuffle_ps(
            asm2, asm2,
            _MM_SHUFFLE(1, 3, 0,
                        2));// [2xy + 2zw, 1 - 2(x^2 + z^2), 2yz - 2xw, 0]

    ysGeneric asm3 = _mm_shuffle_ps(
            calc1, calc2,
            _MM_SHUFFLE(3, 2, 3, 0));// [2xz - 2yw, 2yz + 2xw, 1 - 2(x^2 + y^2)]
    // No need to shuffle this one

    return ysMath::Transpose(
            ysMath::LoadMatrix(asm1, asm2, asm3, Constants::IdentityRow4));
}
...
void ysMath::LoadMatrix(const ysQuaternion &quat, const ysVector &origin,
                        ysMatrix *full, ysMatrix *orientation) {
    ysGeneric q = ysMath::Normalize(quat);// q = |quat|
    ysGeneric nq =
            _mm_sub_ps(ysMath::Constants::Zero, q);// nq = [-w, -x, -y, -z]
    ysGeneric qq = _mm_add_ps(q, q);               // qq = [2w, 2x, 2y, 2z]
    ...
    *orientation =
            Transpose(LoadMatrix(asm1, asm2, asm3, Constants::IdentityRow4));
...
ysMatrix ysMath::OrthogonalInverse(const ysMatrix &m) {
    ...
    ysMatrix r_inv = LoadMatrix(r.rows[0], r.rows[1], r.rows[2],
...
ysMatrix ysMath::Inverse3x3(const ysMatrix &m) {
    ...
    // Calculate row 1
    ysVector r1_0 =
            _mm_shuffle_ps(m.rows[1], m.rows[0], _MM_SHUFFLE(1, 2, 3, 1));
    r1_0 = _mm_shuffle_ps(r1_0, r1_0, _MM_SHUFFLE(1, 3, 2, 0));

    ysVector r1_1 =
            _mm_shuffle_ps(m.rows[2], m.rows[1], _MM_SHUFFLE(3, 2, 1, 2));
    ysVector r1_2 =
            _mm_shuffle_ps(m.rows[1], m.rows[0], _MM_SHUFFLE(2, 1, 3, 2));
    r1_2 = _mm_shuffle_ps(r1_2, r1_2, _MM_SHUFFLE(1, 3, 2, 0));

    ysVector r1_3 =
            _mm_shuffle_ps(m.rows[2], m.rows[1], _MM_SHUFFLE(3, 1, 2, 1));
    ...
    // Calculate row 2
    ysVector r2_0 =
            _mm_shuffle_ps(m.rows[1], m.rows[0], _MM_SHUFFLE(2, 0, 3, 2));
    r2_0 = _mm_shuffle_ps(r2_0, r2_0, _MM_SHUFFLE(1, 3, 2, 0));

    ysVector r2_1 =
            _mm_shuffle_ps(m.rows[2], m.rows[1], _MM_SHUFFLE(3, 0, 2, 0));
    ysVector r2_2 =
            _mm_shuffle_ps(m.rows[1], m.rows[0], _MM_SHUFFLE(0, 2, 3, 0));
    r2_2 = _mm_shuffle_ps(r2_2, r2_2, _MM_SHUFFLE(1, 3, 2, 0));

    ysVector r2_3 =
            _mm_shuffle_ps(m.rows[2], m.rows[1], _MM_SHUFFLE(3, 2, 0, 2));
    ...
    // Calculate row 3
    ysVector r3_0 =
            _mm_shuffle_ps(m.rows[1], m.rows[0], _MM_SHUFFLE(0, 1, 3, 0));
    r3_0 = _mm_shuffle_ps(r3_0, r3_0, _MM_SHUFFLE(1, 3, 2, 0));

    ysVector r3_1 =
            _mm_shuffle_ps(m.rows[2], m.rows[1], _MM_SHUFFLE(3, 1, 0, 1));
    ysVector r3_2 =
            _mm_shuffle_ps(m.rows[1], m.rows[0], _MM_SHUFFLE(1, 0, 3, 1));
    r3_2 = _mm_shuffle_ps(r3_2, r3_2, _MM_SHUFFLE(1, 3, 2, 0));

    ysVector r3_3 =
            _mm_shuffle_ps(m.rows[2], m.rows[1], _MM_SHUFFLE(3, 0, 1, 0));
...
ysMatrix ysMath::Negate4x4(const ysMatrix &m) {
    return ysMath::LoadMatrix(
            ysMath::Negate(m.rows[0]), ysMath::Negate(m.rows[1]),
            ysMath::Negate(m.rows[2]), ysMath::Negate(m.rows[3]));
}

ysMatrix ysMath::Negate3x3(const ysMatrix &m) {
    return ysMath::LoadMatrix(ysMath::Negate3(m.rows[0]),
                              ysMath::Negate3(m.rows[1]),
                              ysMath::Negate3(m.rows[2]), m.rows[3]);
}
...
ysMatrix ysMath::MatMult(const ysMatrix &m1, const ysMatrix &m2) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        r.rows[i] = _mm_madd_ps(_mm_replicate_y_ps(m1.rows[i]), m2.rows[1],
                                r.rows[i]);
        r.rows[i] = _mm_madd_ps(_mm_replicate_z_ps(m1.rows[i]), m2.rows[2],
                                r.rows[i]);
        r.rows[i] = _mm_madd_ps(_mm_replicate_w_ps(m1.rows[i]), m2.rows[3],
                                r.rows[i]);
    }
...
ysMatrix ysMath::MatAdd(const ysMatrix &m1, const ysMatrix &m2) {
    return LoadMatrix(ysMath::Add(m1.rows[0], m2.rows[0]),
...
ysMatrix ysMath::MatConvert3x3(const ysMatrix &m) {
    return LoadMatrix(ysMath::Mask(m.rows[0], ysMath::Constants::MaskOffW),
...
ysMatrix ysMath::MatNormalize(const ysMatrix &m) {
    ...
    return Transpose(LoadMatrix(
            ysMath::Normalize(m_T.rows[0]), ysMath::Normalize(m_T.rows[1]),
            ysMath::Normalize(m_T.rows[2]), ysMath::Constants::IdentityRow4));
}

ysMatrix ysMath::FrustrumPerspective(float fovy, float aspect, float near,
                                     float far) {
...
ysMatrix ysMath::OrthographicProjection(float width, float height, float near,
                                        float far) {
...
ysMatrix ysMath::CameraTarget(const ysVector &eye, const ysVector &target,
                              const ysVector &up) {
...
ysMatrix ysMath::RotationTransform(const ysVector &axis, float angle) {
    ...
    ysMatrix r;
    r.rows[0] = ysMath::LoadVector(cos_a + ux * ux * (1 - cos_a),
                                   ux * uy * (1 - cos_a) - uz * sin_a,
                                   ux * uz * (1 - cos_a) + uy * sin_a);
    r.rows[1] = ysMath::LoadVector(uy * ux * (1 - cos_a) + uz * sin_a,
                                   cos_a + uy * uy * (1 - cos_a),
                                   uy * uz * (1 - cos_a) - ux * sin_a);
    r.rows[2] = ysMath::LoadVector(uz * ux * (1 - cos_a) - uy * sin_a,
...
ysVector ysMath::Clamp(const ysVector &a, const ysVector &r_min,
                       const ysVector &r_max) {
