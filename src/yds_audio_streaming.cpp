...
ysError ysStreamingAudio::AttachAudioFile(ysAudioFile *file)
{
	...
	error = UpdateMetrics();
	if (error != ysError::YDS_NO_ERROR) m_file = NULL;

	return YDS_ERROR_RETURN(error);

}

ysError ysStreamingAudio::AttachAudioBuffer(ysAudioBuffer *buffer)
{
	...
	error = UpdateMetrics();
	if (error != ysError::YDS_NO_ERROR) m_buffer = NULL;
...
ysError ysStreamingAudio::UpdateMetrics()
{
	...
	if (m_file == NULL) return YDS_ERROR_RETURN(ysError::YDS_NO_ERROR);
	if (m_buffer == NULL) return YDS_ERROR_RETURN(ysError::YDS_NO_ERROR);
	if (m_file->GetBufferSize() > m_buffer->GetBufferSize()) return YDS_ERROR_RETURN(ysError::YDS_INCOMPATIBLE_BUFFER_AND_FILE);
...
ysError ysStreamingAudio::Seek(SampleOffset offset)
{
	...
	if (m_file == NULL) return YDS_ERROR_RETURN(ysError::YDS_NO_FILE);
	if (m_buffer == NULL) return YDS_ERROR_RETURN(ysError::YDS_NO_AUDIO_BUFFER);
	if (m_file->GetBufferSize() == 0) return YDS_ERROR_RETURN(ysError::YDS_NO_FILE_BUFFER);

	ysAudioBuffer::AUDIO_BUFFER_MODE mode = m_buffer->GetBufferMode();
	m_buffer->SetMode(ysAudioBuffer::MODE_STOP);
	m_buffer->Seek(0);
